...
namespace Droid_Inventory_Management_Tool_Revised
{
    class DroidCollection : IDroidCollection
    {
        ...
        public void TypeSort()
        {
            ...
            // Loop through collection and add to correct stacks
            foreach (Droid x in droidCollection) 
            {
                if (x != null)
                {
                    Type droidType = x.GetType();

                    if (droidType == typeof(AstromechDroid))
                        AstroStack.Push((AstromechDroid)x);
                    else if (droidType == typeof(JanitorDroid))
                        JanitorStack.Push((JanitorDroid)x);
                    else if (droidType == typeof(UtilityDroid))
                        UtilityStack.Push((UtilityDroid)x);
                    else if (droidType == typeof(ProtocolDroid))
                        ProtocolStack.Push((ProtocolDroid)x);
                }
            }
            ...
            {
                droidCollection[index] = DroidQueue.Dequeue();
                ++index;
            }
        }
    }
